import React, { Component } from 'react'

// export default class DinoList extends Component {

function DinoList() {
    return (
        <div>
            <div className="cover">
                <h1 className="cover-title">
                    <span>The Story of</span>
                    Dinosaur A-Z
                </h1>
            </div>

            <div className="wrapper">
                <ul className="timeline">
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1553363216/ankylosaurus_w6bgsx.jpg" alt="Vector cartoon green dinosaur  png clipart from pngtree.com" />
                            <h2 className="card-title">Ankylosaurus</h2>
                            <dl className="card-definition">
                                <dt>Ankylosaurus<span>Ankylosaurus</span></dt>
                                <dd>Ankylosaurus was the largest armoured dinosaur. They lived during the late Cretatious period. They had boney spikes on their backs to protect themselves from thier predators. They had a ball at the end of their tails.  </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1553372009/baryonyx_rb1xyi.jpg" alt="Baryonyx" />
                            <h2 className="card-title">Baryonyx</h2>
                            <dl className="card-definition">
                                <dt>
                                    Baryonyx
                                    <span>Baryonyx</span>
                                </dt>
                                <dd>Baryonyx means heavy claw. They are carnivores and they love to eat fish the most. They catch their prey (fish) with their claWS.</dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1553442853/corythosaurus_whvft9.jpg" alt="Corythosaurus" />
                            <h2 className="card-title">Corythosaurus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Corythosaurus
                                    <span>Corythosaurus</span>
                                </dt>
                                <dd>The Corythosauras are herbivores. They are from the late cretatious period.</dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1553443259/diplodocus_lngp6v.jpg" alt="Diplodocus" />
                            <h2 className="card-title">Diplodocus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Diplodocus
                                    <span>Diplodocus</span>
                                </dt>
                                <dd> Diplodocus has a very long neck and tail. When predoetors tried to attack, they would swing its tail. This dinosaur is from the late Jurassic period. They are one of the longest dinosaurs in the world. </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1553443519/euoplocephalus_lecln0.jpg" alt="Euoplocephalus" />
                            <h2 className="card-title">Euoplocephalus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Euoplocephalus
                                    <span>Euoplocephalus</span>
                                </dt>
                                <dd> Euoplocephalus are armoured dinosaurs. They have a ball at the end of their tails and they broke predators tails with their boney spikes on their backs. They are from the late cretatous period.   </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1553443711/fabrosaurus_qruwli.jpg" alt="Fabrosaurus" />
                            <h2 className="card-title">Fabrosaurus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Fabrosaurus
                                    <span>Fabrosaurus</span>
                                </dt>
                                <dd>Fabrosaurus are herbivores from the late cretatous period. When they saw predators, they ran very fast away from them. </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1553443914/gorgosaurus.jpq_zgsbkb.jpg" alt="Gorgosaurus" />
                            <h2 className="card-title">Gorgosaurus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Gorgosaurus
                                    <span>Gorgosaurus</span>
                                </dt>
                                <dd> Gorgosaurus are carnivores from the late cretatous. They ate Zuuls and other herbivores. Sometimes their theeth would break when eating other dinosaurs. </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1553444125/herrerasaurus.jpq_z8z1bc.jpg" alt="Herrerasaurus" />
                            <h2 className="card-title">Herrerasaurus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Herrerasaurus
                                    <span>Herrerasaurus</span>
                                </dt>
                                <dd> Herrerasaurus are carnivores from the Triassic period. They ate insects and other herbivores.   </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1553444284/iguanodon_ksypck.jpg" alt="Iguanodon" title="Iguanodon" />
                            <h2 className="card-title">Iguanodon</h2>
                            <dl className="card-definition">
                                <dt>
                                    Iguanodon
                                    <span>Iguanodon</span>
                                </dt>
                                <dd>Iguanodon are herbivores from the cretatious period. They stood on two legs to eat leaves from tall trees. Sometimes they lived in herds. </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://png.pngtree.com/element_pic/16/10/31/8d5628a528a720539da8c606f6677b0d.jpg" alt="Janenschia" title="Janenschia" />
                            <h2 className="card-title">Janenschia</h2>
                            <dl className="card-definition">
                                <dt>
                                    Janenschia
                                    <span>Janenschia</span>
                                </dt>
                                <dd>Janenschia are herbivores. It is an armoured dinosaur with a very long neck.  </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1557708923/kentrosaurus_w5txxz.jpg" alt="Kentrosaurus" title="Kentrosaurus" />
                            <h2 className="card-title">Kentrosaurus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Kentrosaurus
                                    <span>Kentrosaurus</span>
                                </dt>
                                <dd>Kentrosaurus have spikes on their back and a spike on their legs. They are herbivores that lived during both the Jurrasic and Cretatious period.  </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1553819121/leptoceratops_gamqme.jpg" alt="Leptoceratops" title="Leptoceratops" />
                            <h2 className="card-title">Leptoceratops</h2>
                            <dl className="card-definition">
                                <dt>
                                    Leptoceratops
                                    <span>Leptoceratops</span>
                                </dt>
                                <dd>Leptoceratops are herbivores. Leptoceratops means 'slender-horned face' but doesn't have real horned. They can walk four legs or two legs. He is smaller than human.</dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1557708672/minmi_yeerqr.jpg" alt="minmi" />
                            <h2 className="card-title">Minmi</h2>
                            <dl className="card-definition">
                                <dt>
                                    Minmi
                                    <span>Minmi</span>
                                </dt>
                                <dd> Minmi are herbivores from the early Cretatious period. They have boney spikes on their back and armour inside their stomach. They have little sharp teeth.</dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1557706245/nigersaurus_jravxr.jpg" alt="Nigersaurus" />
                            <h2 className="card-title">Nigersaurus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Nigersaurus
                                    <span>Nigersaurus</span>
                                </dt>
                                <dd> Nigersaurus are herbivores from the late Jurassic period. Nigersaurus is also sauropods. Sauropods was a long neck dinosaur. You cannot miss them because they're so large! They're longer than three school buses.   </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://png.pngtree.com/element_pic/16/10/31/8d5628a528a720539da8c606f6677b0d.jpg" alt="Vector cartoon green dinosaur  png clipart from pngtree.com" />
                            <h2 className="card-title">Oviraptor</h2>
                            <dl className="card-definition">
                                <dt>
                                    Oviraptor
                                    <span>Oviraptor</span>
                                </dt>
                                <dd> Oviraptors were carnivores that did not have teeth. They had storng jaws. They kept their eggs warm with their feathers. </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1553817652/pachycephalosaurus.jpq_oggebb.jpg" alt="Pachycephalosaurus" title="Pachycephalosaurus" onTouchStart={() => { console.log('onClick') }} />
                            <h2 className="card-title">Pachycephalosaurus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Pachycephalosaurus
                                    <span>Pachycephalosaurus</span>
                                </dt>
                                <dd>Pachycephalosaurus had thick heads. Their heads were as thick as a bone. They banged their predator's heads. These dinosauars walked on two legs and had a curvey "s" shaped neck.  </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://png.pngtree.com/element_pic/16/10/31/8d5628a528a720539da8c606f6677b0d.jpg" alt="Vector cartoon green dinosaur  png clipart from pngtree.com" />
                            <h2 className="card-title">Quetzalcoatlus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Quetzalcoatlus
                                    <span>Quetzalcoatlus</span>
                                </dt>
                                <dd>Quetzalcoatlus were large flying reptiles. They lived during the same time as dinosaur. They are fish, and sometimes dead dinosaurs too.</dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://png.pngtree.com/element_pic/16/10/31/8d5628a528a720539da8c606f6677b0d.jpg" alt="Vector cartoon green dinosaur  png clipart from pngtree.com" />
                            <h2 className="card-title">Rebbachisaurus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Rebbachisaurus
                                    <span>Rebbachisaurus</span>
                                </dt>
                                <dd>Rebbachisaurus are one of the oldest herbivores. They had five claws. They are from the Triassic period. </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://png.pngtree.com/element_pic/16/10/31/8d5628a528a720539da8c606f6677b0d.jpg" alt="Vector cartoon green dinosaur  png clipart from pngtree.com" />
                            <h2 className="card-title">Stegosaurus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Stegosaurus
                                    <span>Stegosaurus</span>
                                </dt>
                                <dd> Stegosaurus were herbivores. They had a plate on their back which kept them warm. When predators attacked, they swung their spikes on their tails, and turned the colour of their plates red. This dinosaur is from the Jurassic period. </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://png.pngtree.com/element_pic/16/10/31/8d5628a528a720539da8c606f6677b0d.jpg" alt="Vector cartoon green dinosaur  png clipart from pngtree.com" />
                            <h2 className="card-title">Tyrannosaurus Rex</h2>
                            <dl className="card-definition">
                                <dt>
                                    Tyrannosaurus Rex
                                    <span>Tyrannosaurus Rex</span>
                                </dt>
                                <dd>Tyrannosaurus Rex, commonly known as the T Rex, were carnivores from the late Cretatious periof. They ate many different dinosaurs. They fought the Ankylosaurus for their prey. </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1553559987/ultrasaurus_iyag0h.jpg" alt="Ultrasaurus" />
                            <h2 className="card-title">Ultrasaurus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Ultrasaurus
                                    <span>Ultrasaurus</span>
                                </dt>
                                <dd>Ultrasaurus were herbivores. They were one of the biggest dinosaurs and reachable third floor building. They were 20 times bigger than a person. His front legs is longer than back legs</dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://png.pngtree.com/element_pic/16/10/31/8d5628a528a720539da8c606f6677b0d.jpg" alt="Vector cartoon green dinosaur  png clipart from pngtree.com" />
                            <h2 className="card-title">Velociraptor</h2>
                            <dl className="card-definition">
                                <dt>
                                    Velociraptor
                                    <span>Velociraptor</span>
                                </dt>
                                <dd>Velociraptor are carnivores from the late Cretatious period. They were very fast. They had very sharp claws. Because they were not strong dinosaurs, they caught their prey in heards.</dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://png.pngtree.com/element_pic/16/10/31/8d5628a528a720539da8c606f6677b0d.jpg" alt="Vector cartoon green dinosaur  png clipart from pngtree.com" />
                            <h2 className="card-title">Wannanosauurus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Wannanosauurus
                                    <span>Wannanosauurus</span>
                                </dt>
                                <dd>Wannanosauurus liked to play head banging games with other Wannanosauuruses. They were herbivores and ate plates from the ground. They are from the late Cretatious period. When predators attacked, they would head-butt their predators. </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://png.pngtree.com/element_pic/16/10/31/8d5628a528a720539da8c606f6677b0d.jpg" alt="Vector cartoon green dinosaur  png clipart from pngtree.com" />
                            <h2 className="card-title">Yingshanosaurus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Yingshanosaurus
                                    <span>Yingshanosaurus</span>
                                </dt>
                                <dd>Yingshanosaurus are from the Jurassic period. They had plates on their backs which kept them warm. They at ate plant from the grounds. When predators came, the Yingshanosaurus swung their four-spiked tails at them. </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://png.pngtree.com/element_pic/16/10/31/8d5628a528a720539da8c606f6677b0d.jpg" alt="Vector cartoon green dinosaur  png clipart from pngtree.com" />
                            <h2 className="card-title">Xenotarsosaurus</h2>
                            <dl className="card-definition">
                                <dt>
                                    Xenotarsosaurus
                                    <span>Xenotarsosaurus</span>
                                </dt>
                                <dd>Xenotarsosaurus were carnivores. They had two hords above their eyes. They looked like an armoured dinosaur but they actually were not. </dd>
                            </dl>
                        </div>
                    </li>
                    <li className="timeline-item">
                        <div className="card">
                            <img className="card-icon" src="https://res.cloudinary.com/drrwovgv6/image/upload/v1553362488/zuul_adaw57.jpg" alt="Zuul" />
                            <h2 className="card-title">Zuul</h2>
                            <dl className="card-definition">
                                <dt>
                                    Zuul
                                    <span>Zuul</span>
                                </dt>
                                <dd>Zuuls were armoured dinosaurs from the late Cretatious period. They had boney spikes on his back and a ball at the end of their tails. The name Zuul came from the Red Eye Monster. The stomach of the Zuul is not armoured. They have boney epikes on their face.</dd>
                            </dl>
                        </div>
                    </li>
                </ul>
            </div>

        </div>
    )
}
export default DinoList;
